...
func (n *NodeChecker) Check(ctx context.Context, checkOpts CheckArgs) ClusterCheckResult {
	...
	if err != nil {
		...
		return MakeClusterDegradedError(CheckStatusOpenshiftAPIError, reason)
	}
	...
	responses := n.waitForWorkFinish()
	if len(responses) == 1 {
		return responses[0]
	} else {
		for _, response := range responses {
			klog.Errorf("Checking for node failed with %s", response.Reason)
		}
		return responses[0]
	}
}
...
func (n *NodeChecker) getHost(ctx context.Context, checkOpts CheckArgs, hostRef *types.ManagedObjectReference) (mo.HostSystem, error) {
	var o mo.HostSystem
	hostName := hostRef.Value
	hostSystemObject := object.NewHostSystem(checkOpts.vmConnection.Client, *hostRef)
