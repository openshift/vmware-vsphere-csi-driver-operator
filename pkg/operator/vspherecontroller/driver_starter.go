...
func (c *VSphereController) createCSIDriver() {
	csiControllerSet := csicontrollerset.NewCSIControllerSet(
		...
		WithMaxVolumesPerNodeDeploymentHook(c.apiClients.ClusterCSIDriverInformer.Lister()),
		...
		WithMaxVolumesPerNodeDaemonSetHook(c.apiClients.ClusterCSIDriverInformer.Lister()),
...
func setMaxVolumesPerNodeEnvVar(containers []v1.Container, clusterCSIDriverLister operatorlistersv1.ClusterCSIDriverLister) ([]v1.Container, error) {
	// Default value for vSphere 7 hosts
	maxVolumesPerNode := int32(59)
...
func WithMaxVolumesPerNodeDaemonSetHook(clusterCSIDriverLister operatorlistersv1.ClusterCSIDriverLister) csidrivernodeservicecontroller.DaemonSetHookFunc {
	return func(opSpec *operatorapi.OperatorSpec, ds *appsv1.DaemonSet) error {
		containers, err := setMaxVolumesPerNodeEnvVar(ds.Spec.Template.Spec.Containers, clusterCSIDriverLister)
		if err != nil {
			return err
		}
...
func WithMaxVolumesPerNodeDeploymentHook(clusterCSIDriverLister operatorlistersv1.ClusterCSIDriverLister) deploymentcontroller.DeploymentHookFunc {
	return func(opSpec *operatorapi.OperatorSpec, deployment *appsv1.Deployment) error {
		containers, err := setMaxVolumesPerNodeEnvVar(deployment.Spec.Template.Spec.Containers, clusterCSIDriverLister)
		if err != nil {
			return err
		}
		deployment.Spec.Template.Spec.Containers = containers
